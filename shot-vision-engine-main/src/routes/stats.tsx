...
import {
  C, F, NUM, Bar, Card, Chips, CourtWatermark, DEFAULT_ACCENT, EmptyState,
  ErrorState, GhostButton, NavLink, RankPill, SearchInput, SectionLabel,
  Segmented, TableSkeleton, TeamBackdrop, TeamChip, TeamPageTint, accentVars,
  teamColor, tierColor,
} from "../components/stat-engine/ui";
import {
  Column, Row, Stat, PlayerProfile, TeamProfile,
  buildDistributions, formatStat, isRankable, matchesQuery, numericValue,
  percentileOf, rankOf, foldAccents, sortRows, LOWER_IS_BETTER,
...
function StatEnginePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([get("/stats/players"), get("/stats/teams")])
      .then(([p, t]) => {
        if (cancelled) return;
  ...
  return (
    <div style={{
      ...
      {themeAbbrev && <TeamPageTint abbrev={themeAbbrev} />}
      ...
      <header style={{
        ...
        <nav style={{ display: "flex", gap: 22, alignItems: "center", position: "relative" }}>
          ...
          <NavLink href="/model-features">MODEL FEATURES</NavLink>
...
function useRanking(columns: Column[], rows: Row[], sortKey: string) {
  ...
  const distribution = useMemo(
    () => (rankingBy ? buildDistributions(rows, [rankingBy.key]).get(rankingBy.key) ?? [] : []),
    [rows, rankingBy]
  );
  return { rankingBy, distribution };
}

function PlayerLeaderboard({ columns, rows, teamNames, onOpen }: {
  columns: Column[]; rows: Row[]; teamNames: Map<string, string>; onOpen: (id: string) => void;
}) {
  ...
  const filtered = useMemo(() => {
    ...
    const base = q ? rows.filter((r) => matchesQuery(String(r.name ?? ""), q)) : rows;
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        <Chips options={groups} value={active} onChange={setGroup} />
      </div>
      <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.5 }}>
          {rankingBy ? (
            <>
              Ranked by <span style={{ color: C.gold }}>{rankingBy.label}</span> across all{" "}
              {distribution.length} players it is measured for — a name search keeps the league
              rank instead of renumbering.
            </>
          ) : (
      ...
      <Card style={{ overflow: "hidden" }}>
        <div className="se-scroll">
          <table className="se-table">
            ...
            <tbody>
              {filtered.map((r, i) => {
                const value = rankingBy ? numericValue(r[rankingBy.key]) : null;
                ...
                return (
                  <tr key={String(r.player_id)} onClick={() => onOpen(String(r.player_id))}>
                    ...
                    <td className="se-stick se-col-name se-name se-left">{String(r.name ?? "—")}</td>
...
function PlayerProfileView({ playerId, leagueRows, teamNames, onBack, onOpenTeam }: {
  playerId: string;
  leagueRows: Row[];
  ...
  // league without re-scanning the whole table.
  ...
  return (
    <>
      ...
      {/* Columns, not grid: the sections are wildly different heights
          (Shooting has 16 rows, Overview 6) and a grid leaves a tall hole
          under the short ones. CSS columns pack them. */}
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(290px, 1fr) minmax(330px, 1.25fr)", gap: 16, alignItems: "start" }}>
        <Card style={{ padding: "15px 19px 9px" }}>
          <div style={{ marginBottom: 12 }}><SectionLabel>Team stats</SectionLabel></div>
          {teamCols.map((c) => (
            <StatRow
              key={c.key}
              stat={{
            ...
            <tbody>
              {profile.roster.map((p) => (
                <tr key={p.player_id} onClick={() => onOpenPlayer(p.player_id)}>
                  <td className="se-name se-left" style={{ padding: "7px 12px" }}>{p.name}</td>
