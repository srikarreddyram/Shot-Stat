...
This used to be one 669-line spec.py. It is now:
    classification.py — the mechanic/finish/possession-origin rule-based
                        classifiers
    spatial.py         — the radial spatial basis
    groups.py          — FEATURE_GROUPS
    __init__.py         — this file: derive_features and friends

This __init__.py re-exports everything so every existing
`from src.features.spec import X` keeps working unchanged.
"""
...
from .classification import (
    DEFAULT_POSSESSION_ORIGIN,
    FINISH_RULES,
    FINISH_TYPES,
    POSSESSION_ORIGIN_RULES,
    POSSESSION_ORIGINS,
    SHOT_MECHANIC_FALLBACK,
    SHOT_MECHANIC_RULES,
    SHOT_MECHANICS,
    classify_finish,
    classify_mechanic,
    classify_possession_origin,
)
from .groups import FEATURE_GROUPS
from .spatial import SPATIAL_CENTERS, SPATIAL_FEATURES, SPATIAL_SIGMA_FT
...
def derive_features(
    ...
    if {"quarter", "time_remaining", "score_diff"}.issubset(out.columns):
        # The NBA's own official "Clutch Time" window (last 5 minutes of the
        # 4th or overtime, score within 5) — CLUTCH_TIME_REMAINING/
        # CLUTCH_MARGIN live in point_in_time.clutch_performance so this flag
        # and that module's career clutch-shooting counts describe the
        # identical window rather than two different notions of "clutch".
        # (Previously a stricter, ad hoc 2-minute window with no particular
        # basis; widened to match here rather than narrowing the new feature
        # to match the old flag, since 5 minutes is the definition the league
        # itself publishes and measures against.)
        out["clutch_flag"] = (
            (out["quarter"] >= 4)
            & (out["time_remaining"] <= CLUTCH_TIME_REMAINING)
            & (out["score_diff"].abs() <= CLUTCH_MARGIN)
        ).astype(int)
        if "clutch_fg_delta" in out.columns:
            # Zero on every non-clutch shot, engaged only when the shot
            # itself is clutch — see point_in_time.clutch_performance's
            # module docstring for why this is handed to the trees
            # pre-multiplied rather than left as two separate columns.
            out["clutch_edge"] = out["clutch_fg_delta"] * out["clutch_flag"]
...
__all__ = [
    "ZONE_SUFFIX", "ZONES", "ZONE_TO_DEF_CATEGORY",
    "INTERIOR_ZONES", "REQUIRED_RAW_COLS",
    "FEATURE_GROUPS",
    "SHOT_MECHANIC_RULES", "SHOT_MECHANIC_FALLBACK", "SHOT_MECHANICS",
    "FINISH_RULES", "FINISH_TYPES",
    "POSSESSION_ORIGIN_RULES", "POSSESSION_ORIGINS", "DEFAULT_POSSESSION_ORIGIN",
    "SPATIAL_SIGMA_FT", "SPATIAL_CENTERS", "SPATIAL_FEATURES",
    "classify_finish", "classify_possession_origin", "classify_mechanic",
    "all_feature_columns", "derive_features", "as_model_matrix", "interior_mask",
]
